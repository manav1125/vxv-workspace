...
type PanelState = "execution" | "artifact" | "workspace" | null;
...
function App() {
  ...
  const handleNodeAction = (node: ThreadNode) => {
    ...
    if (node.kind === "run") {
      setPanel(linkedExecution ? "execution" : "workspace");
    }
  };
        ...
        <div className={`chat-layout ${memoryOpen ? "with-memory" : "without-memory"}`}>
                ...
                {messages.map((message) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {nodes.length ? (
                        <div className="node-strip">
                          {nodes.map((node) => (
                            <button
                              key={node.id}
                              className={`node-pill node-${node.kind}`}
                              onClick={() => handleNodeAction(node)}
                            >
                              <span className="node-pill-kind">{node.kind}</span>
                      ...
                      {nodes.some((node) => node.kind === "approval" && node.task_run_id) ? (
                        <div className="thread-approval-actions">
                          {nodes
                            .filter((node) => node.kind === "approval" && node.task_run_id)
                            .map((node) => (
                              <div key={node.id} className="approval-inline-card">
                                <p>{node.summary}</p>
          ...
          {panel ? (
            <aside className="workspace-dock">
              <div className="workspace-panel workspace-panel-docked">
                {panel === "execution" ? (
                  <>
                    <div className="panel-header">
                      <div>
                        <p className="eyebrow">Working panel</p>
                        <h2>{selectedApp?.title ?? "Current execution"}</h2>
                      </div>
                    ...
                    {selectedExecution ? (
                      ...
                      <p className="panel-summary">The workspace will open the current run here after you ask it to do work.</p>
                    )}
                  </>
                ) : null}
                {panel === "artifact" && selectedArtifact ? (
                  <>
                    <div className="panel-header">
                      ...
                      <div className="dock-header-actions">
                        <button className="button-secondary" onClick={() => setPanel("execution")}>
                          Back to run
                        </button>
                        <button className="button-secondary" onClick={() => setPanel(null)}>
                          Close
